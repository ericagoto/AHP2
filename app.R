...
server <- function(input, output) {
  ...
  Angulo <-reactive( {
    if (x() ==  "A10") { y <- 0.6336}
    if (x() == "A10_17") {y<- 0.999}
    if (x() == "A_17_30") {y<-1.798}
    if (x() == "A_30_60") {y<-3.015}
    if (x() == "A_60_90") {y<-5.685}
    if (x() == "A90") {y <- 5.848}
    y
  })
  ...
  solo <- reactive ({
    ...
    if (x_solo() == "favoravel") {solo <-  4.720 }
    if (x_solo() == "desfav") {solo <- 0}
    if (x_solo() == "nao_obs") {solo <- (2.360)}
  ...
  cobertura_natural <- reactive ({
    ...
    if (x_cob_nat() == "favoravel") {cobertura_natural <-  1.881 }
    if (x_cob_nat() == "desfav") {cobertura_natural <- (0)}
    if (x_cob_nat() == "nao_obs") {cobertura_natural <- 0.941}
  ...
  Geologia <- reactive ({
    ...
    if (x_geol() == "favoravel") {geologia <-  4.651 }
    if (x_geol() == "desfav") {geologia <- 0}
    if (x_geol() == "nao_obs") {geologia <- (2.326)}
  ...
  Moradia <-reactive ({
    ...
    if (x1() == TRUE & x2() == FALSE & x3() == FALSE) {moradia = 0.775 } # alvenaria
    if (x1() == FALSE & x2() == TRUE & x3() == FALSE) {moradia = 3.152 } # madeira
    if (x1() == FALSE & x2() == FALSE & x3() == TRUE) {moradia = 3.595} # mista
   
    
    
    ### NEED to fix input as Percentage
     if (x1() == TRUE & x2() == TRUE & x3() == FALSE) {moradia = (0.775+ 3.152)/2} #alvenaria & madeira
    if (x1() == TRUE & x2() == FALSE & x3() == TRUE)  {moradia = (0.775+3.595)/2} # alvenaria e mista
    if (x1() == FALSE & x2() == TRUE & x3() == TRUE)  {moradia = (3.152+3.595)/2}# madeira e mista
    if (x1() == TRUE & x2() == TRUE & x3() == TRUE)   {moradia = (0.775+3.152+3.595)/3} # alvenaria, madeira e mista
  ...
  Material <- reactive ({
    ...
    if (a() == TRUE & l()==FALSE & e()==FALSE) {material = 3.878} # so aterro
    if (l() == TRUE  & a()==FALSE & e()==FALSE) {material = 3.878} # so lixo
     if (e() == TRUE & a() == FALSE & l()==FALSE) {material = 3.878} # so entulho
    if (a() == TRUE & l()==TRUE & e()==FALSE) {material = 3.878} #  aterro & lixo
    if (l() == TRUE  & a()==FALSE & e()==TRUE) {material = 3.878} #  lixo & entulho
    if (e() == TRUE & a() == TRUE & l()==FALSE) {material = 3.878} #  entulho & aterro
    if (e() == TRUE & a() == TRUE & l()==TRUE) {material = 3.878} #  entulho & aterro & lixo
  ...
  Densidade <- reactive ({
    ...
    if (densidade() == "densidade1") { densidade <- 0.952 }
    if (densidade() == "densidade2") { densidade <- 2.550}
    if (densidade() == "densidade3") { densidade <- 2.961 }
    if (densidade() == "densidade4") { densidade <- 2.271}
  ...
  Agua <- reactive ({
    ...
    if (servida() == "TRUE") {servida = 0.961} 
    else if (servida() == "FALSE") {servida = 0} 
   
    if (conc_chuva() == "TRUE") {conc_chuva = 0.977}
    else if (conc_chuva() == "FALSE") {conc_chuva = 0}
  
    if (vazamento() == "TRUE") {vazamento = 1.949}
    else  if (vazamento() == "FALSE") {vazamento = 0}
  
    if (fossa() == "TRUE") {fossa = 0.839}
    else if (fossa() == "FALSE") {fossa = 0}

    
    if (drenagem() == "inexistente") {drenagem = 0.624}
    if (drenagem() == "precario") { drenagem = 0.535}
    if (drenagem() == "satisf") {drenagem = 0.099}
  ...
  Vegetacao <- reactive ({
    ...
    cobertura_urbana=0
    if(arvore() == "TRUE"){arvore <- 0.370}
    else if (arvore() == "FALSE"){arvore <- 0}
    
    if(arbusto() == "TRUE"){arbusto <- 0.347}
    else if (arbusto() == "FALSE"){arbusto <- 0}
    
    if(rasteira() == "TRUE" ) {rasteira <- 0.405}
    else   if(rasteira() == "FALSE" ) {rasteira <- 0}
    
    if (desmatado() == "TRUE" ) {desmatado <- 1.137}
    else     if (desmatado() == "FALSE" ) {desmatado <- 0}

    if (bananeira() == "TRUE")(bananeira <- 1.253)
    else if  (bananeira() == "FALSE")(bananeira <- 0)
    
    if (cobertura_urbana() == "TRUE")(cobertura_urbana <- 0.339)
  ...
  #cicatriz=0
    ...
    if(embarricado() == "TRUE"){embarricado = 9.533}
   else if (embarricado() == "FALSE") {embarricado = 0 }
    
    if(trinca_moradia() == "TRUE" ) {trinca_moradia = 11.016}
  else if (trinca_moradia() == "FALSE") {trinca_moradia=0}
    
    if (trinca_terreno() == "TRUE" ) {trinca_terreno = 11.016}
  else if (trinca_terreno() == "FALSE" ) {trinca_terreno=0}
    
    if (inclinados() == "TRUE") {inclinados = 9.533}
  else if (inclinados() == "FALSE")  {inclinados =0}
    
    if (degrau() == "TRUE") {degrau = 16.877}
  else if (degrau() == "FALSE") {degrau=0}
    
    if (cicatriz() == "TRUE"){cicatriz = 12.640}
  else if (cicatriz() == "FALSE") {cicatriz=0}
    
    instabilidade <- embarricado +inclinados+degrau+cicatriz+(trinca_terreno + trinca_moradia)/2
  ...
  Posicao_moradia <- reactive( {
    ...
    if (posicao() == prox_base){prox_base <- 2.145 }
    if (posicao() == dist_base){dist_base <-  0.349}
    if (posicao() == meio_encosta){meio_encosta <-  0.349}
    if (posicao() == prox_topo) {prox_topo <- 1.504}
    if (posicao() == dist_topo) {dist_topo <-0.286}
    if (posicao() == encosta_tudo ) {encosta_tudo <-0.349+ 1.504 + 2.145}
  ...
  risk <-reactive ({
    ...
    if (final < 26) {return("R1")}
    if (final >= 26 & final < 42) {return("R2")}
    if (final >= 42 & final < 67) {return("R3")}
    if (final >= 67) {return("R4")}
    final
  })   
